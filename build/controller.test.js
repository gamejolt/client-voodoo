...
describe('Joltron Controller', function () {
    ...
    function newController(port, options) {
        options = options || {};
        options.connectionTimeout = 3000; // 3 seconds to keep tests manageable.
    ...
    it('should attach to running instance', async () => {
    ...
    it('should wait until connection is made', async () => {
        let connected = false;
        mockRunner(socket => {
    ...
    it('should connect only once while connected', async () => {
        let connectCount = 0;
        mockRunner(socket => {
            connectCount++;
        });
    ...
    it('should connect only once even when parallel', async () => {
        let connectCount = 0;
        mockRunner(socket => {
            connectCount++;
        });
    ...
    it('should wait until disconnection is complete', async () => {
        let connected = true;
        mockRunner(socket => {
    ...
    it('should not fail disconnecting twice in parallel', async () => {
        let disconnectCount = 0;
        mockRunner(socket => {
            socket.on('close', (hasError) => {
                if (hasError) {
    ...
    it('should connect and disconnect in quick succession', async () => {
    ...
    it('should disconnect and connect in quick succession', async () => {
    ...
    it('should retry the initial connection up to 3 seconds', async () => {
    ...
    it('should timeout the connection attempt if over 3 seconds', async () => {
    ...
    it('should emit a "fatal" event when joltron disconnects unexpectedly', async () => {
        const inst = newController(1337);
    ...
    it('should retry connection if the controller is set to keep connection alive', async () => {
    ...
    it('should not retry connection if the controller is not set to keep connection alive', async () => {
        const inst = newController(1337);
        let connections = 0;
        let resolveConnected = null;
        const waitForConnect = new Promise(_resolve => {
    ...
    function getMockReaderPromise(expectedData, expectedResult) {
        return new Promise((resolve, reject) => {
            ...
            mockRunner(socket => {
                const incomingJson = JSONStream.parse(true);
                incomingJson
                    .on('data', data => {
    ...
    it('should send get state command', async () => {
        const mockPromise = getMockReaderPromise({
            type: 'state',
            msgId: '0',
            payload: {
    ...
    it('should send get state command (2)', async () => {
        const mockPromise = getMockReaderPromise({
            type: 'state',
            msgId: '0',
            payload: {
    ...
    it('should send check for updates command', async () => {
        const mockPromise = getMockReaderPromise({
    ...
    it('should send check for updates command (2)', async () => {
        const mockPromise = getMockReaderPromise({
            type: 'checkForUpdates',
            msgId: '0',
            payload: {
    ...
    it('should send update available command', async () => {
        const mockPromise = getMockReaderPromise({
    ...
    it('should send update begin command', async () => {
        const mockPromise = getMockReaderPromise({
            type: 'updateBegin',
            msgId: '0',
            payload: {},
    ...
    it('should send update apply command', async () => {
        const mockPromise = getMockReaderPromise({
            type: 'updateApply',
            msgId: '0',
            payload: {
    ...
    it('should wait for send to get the response', async () => {
        const mockPromise = getMockReaderPromise({
            type: 'updateBegin',
            msgId: '0',
            payload: {},
    ...
    it('should send two messages one after the other', async () => {
        const mockPromise = getMockReaderPromise([
            {
                type: 'updateBegin',
                msgId: '0',
    ...
    it('should send the response as soon as connected', async () => {
        const mockPromise = getMockReaderPromise({
            type: 'updateBegin',
            msgId: '0',
            payload: {},
    ...
    it('should fail sending a message if not connected past the delay', async () => {
        const inst = newController(1337);
    ...
    it('should fail receiving an invalid json', async () => {
        const mockPromise = new Promise((resolve, reject) => {
            mockRunner(socket => {
                socket.write('this is not a valid json', (err) => {
                    if (err) {
                        reject(err);
                        return;
                    }
